...
async def async_write(_data: list, _dataclass: dataclasses.dataclass()) -> list:
    ...
    try:
        # todo: replace -> read /write with a copy (free up memory because (chunks of 100 files) )
        # make directories if needed
        await aiofiles.os.makedirs(variable_tree, exist_ok=True)
        if await aiofiles.os.path.exists(variable_tree):

            # Compatible for Windows and Linux because aiofiles does not support aiofiles.copy() on Windows
            # However: This method requires more memory (chunks=100) so replace with a better one size fits all method.
            # read source file
            async with aiofiles.open(_data[0], mode='rb') as handle:
                handle_read = await handle.read()
            # write destination files
            async with aiofiles.open(_data[3], mode='wb') as handle:
                await handle.write(handle_read)
...
if __name__ == '__main__':
    ...
    if '-h' in stdin:
        ...
        #          and -d some of -d path. dangerous. example:
        #
        #          Command:     -s 'D:\Documents\Work\1. Projects\' -d 'X:\Documents\Work\1. Projects\'
        #
        #          Result:  D:\Documents\Work\1. Projects" -d X:\Documents\Work\1.\
        #
        source = None
        if '-s' in stdin:
            # get source path
