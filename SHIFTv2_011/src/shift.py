...
def canonical_caseless(text: str) -> str:
...
def _remove_dirs(_data: list, _dataclass: dataclasses.dataclass) -> list:
    ...
    if not os.path.exists(hypo_path):
        try:
            ...
            _data.append(f'[FileNotFoundError] [_remove_dirs] {str(e)}')
        except PermissionError as e:
            if _dataclass.debug is True:
                print(f'{get_dt()} [PermissionError] [_remove_dirs] {_path} {str(e)}')
            _data.append(f'[PermissionError] [_remove_dirs] {str(e)}')
        except OSError as e:
            if _dataclass.debug is True:
                print(f'{get_dt()} [OSError] [_remove_dirs] {_path} {str(e)}')
            _data.append(f'[OSError] [_remove_dirs] {str(e)}')
...
async def main(_dataclass: dataclasses.dataclass):
    ...
    _copied_final_results, _updated_final_results, _deleted_final_results, _deleted_dirs_final_results = [], [], [], []
    _copied, _updated, _deleted, _failed_copy, _failed_update, _failed_delete = [], [], [], [], [], []
    _deleted_dirs, _failed_deleted_dirs = [], []
    ...
    if _dataclass.mode == int(0) or _dataclass.mode == int(1) or _dataclass.mode == int(2):
        ...
        if _dataclass.ignore_failed is False:
            [print(f'{get_dt()} {cprint.color(s="[FAILED] [SCAN]", c="R")} {cprint.color(s=failed_task, c=c_data)}') for failed_task in _files_src[1]]
    ...
    if _dataclass.mode == 2:
        ...
        if _dataclass.ignore_failed is False:
            [print(f'{get_dt()} {cprint.color(s="[FAILED] [SCAN]", c="R")} {cprint.color(s=failed_task, c=c_data)}') for failed_task in _files_dst[1]]
            [print(f'{get_dt()} {cprint.color(s="[FAILED] [SCAN]", c="R")} {cprint.color(s=failed_task, c=c_data)}') for failed_task in _directories_dst[1]]
        ...
        if await asyncio.to_thread(out_of_disk_space, _path=dir_dst, _size=total_sz) is False:
            ...
            if await user_accept(_dataclass=_dataclass) is True:
                ...
                if _dataclass.mode == int(2):
                    ...
                    # delete directories
                    [_deleted_dirs_final_results.append(_remove_dirs(_data=[item_dir], _dataclass=_dataclass)) for item_dir in _directories_dst[0]]
                    # todo: uncomment to sort and filter deleted directory results (partially completed)
                    # _deleted_dirs_final_results[:] = [item for sublist in _deleted_dirs_final_results for item in sublist if item is not None]
                    # [_deleted_dirs.append(sublist) for sublist in _deleted_dirs_final_results if sublist[-1] is True]
                    # [_failed_deleted_dirs.append(sublist) for sublist in _deleted_dirs_final_results if sublist[-1] is not True]
                ...
                if _dataclass.ignore_failed is False:
                    ...
                    [print(f'{get_dt()} {cprint.color(s="[FAILED DELETE FILE]", c="R")} {cprint.color(s=failed_task[-1], c=c_data)}') for failed_task in _failed_delete]
                    # todo: uncomment view failed delete directory results (partially completed)
                    # [print(f'{get_dt()} {cprint.color(s="[FAILED DELETE DIRECTORY]", c="R")} {cprint.color(s=failed_task, c=c_data)}') for failed_task in _failed_deleted_dirs]
            else:
